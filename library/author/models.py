...
class Author(models.Model):
    ...
    def delete_by_id(author_id):
        ...
        """
        req = Author.get_by_id(author_id)
        if req:
    ...
    def create(name, surname, patronymic):
        ...
        if len(name) > 20 or len(surname) > 20 or len(patronymic) > 20:
            return None
        new_data = Author(name=name, surname=surname, patronymic=patronymic)
        new_data.save()
        return new_data
    ...
    def to_dict(self):
        ...
        | }
        """
        return {'id': self.pk, 'name': self.name, 'surname': self.surname, 'patronymic': self.patronymic}
